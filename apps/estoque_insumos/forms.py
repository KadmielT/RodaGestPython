...
class InsumoForm(forms.ModelForm):
    ...
    class Meta:
        model = Insumo
        fields = ['nome', 'quantidade', 'valor_unitario', 'unidade_medida', 'codigo', 'descricao']
        widgets = {
            ...
            'codigo': forms.TextInput(attrs={
                'class': 'rg-input',
                'placeholder': 'Digite o código do insumo (opcional)',
            }),
            'descricao': forms.Textarea(attrs={
                'class': 'rg-input',
                'rows': 6,
                'placeholder': 'Digite a descrição do insumo (opcional)',
            }),
        }
        labels = {
            'nome': 'Nome do insumo',
